...
```text
使用 youtube-wiki，帮我把这个 YouTube 链接跑进 skkeoriw/wiki-sop-210-smoke: https://www.youtube.com/watch?v=dQw4w9WgXcQ
```
...
bash <(curl -fsSL https://skill.vyibc.com/youtube-wiki.sh) \
  ...
  --mode=trigger \
  --repo=skkeoriw/wiki-sop-210-smoke \
...
```bash
youtube-wiki trigger --repo skkeoriw/wiki-sop-210-smoke --url 'https://www.youtube.com/watch?v=5MgBikgcWnY'
youtube-wiki trigger --repo skkeoriw/wiki-sop-210-smoke --url 'https://www.youtube.com/watch?v=5MgBikgcWnY' --watch
youtube-wiki status --repo skkeoriw/wiki-sop-210-smoke --pipeline-id '<pipeline_id>'
